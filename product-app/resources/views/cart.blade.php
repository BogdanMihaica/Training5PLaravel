        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 px-4">
            @foreach ($products as $product)
                <div
                    ...
                    <p class="text-lg font-bold text-black-600 mt-4">{{ __('messages.quantity') }}
                        {{ session('cart')[$product->getKey()] }}
                    </p>
                    <a class="mt-4 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition cursor-pointer"
                        href="{{ route('cart.remove', $product->getKey()) }}">{{ __('messages.remove') }}</a>
                </div>
            @endforeach
        </div>
            ...
            <form class="space-y-4" method="POST" action="{{ route('orders.store') }}">
                ...
                <div>
                    ...
                    <input type="text" id="name"
                        ...
                        placeholder="{{ __('messages.enter_name') }}" name="name" value="{{ old('name') }}">
                ...
                <button type="submit"
                    class="w-full bg-slate-600 cursor-pointer hover:bg-slate-500 text-slate-100 py-3 rounded-lg font-semibold transition">{{ __('messages.done') }}</button>
            </form>
        </div>
    </div>
</x-layout>
